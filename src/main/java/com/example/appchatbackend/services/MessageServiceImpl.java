package com.example.appchatbackend.services;
import com.example.appchatbackend.repositories.MessageRepository;
import com.example.appchatbackend.models.Message;
...
public class MessageServiceImpl implements MessageService {
    ...
    @Override
    public Message reactToMessage(String messageId, String userId, String emoji) {
        ...
        if (optMessage.isPresent()) {
            ...
            if (emoji == null || emoji.isBlank()) {
                message.getReactions().remove(userId); // Xóa reaction
            } else {
                message.getReactions().put(userId, emoji); // Thêm/Sửa reaction
            }
            return messageRepository.save(message);
        }
        return null; // Hoặc throw exception tùy ý, ở đây return null, controller sẽ handle throw NotFound trước đó rồi
    }
}
